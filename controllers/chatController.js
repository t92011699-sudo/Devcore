...
const createChatRoom = async (req, res) => {
    try {
        console.log('📥 [createChatRoom] Body:', req.body);
...
const sendMessage = async (req, res) => {
    try {
        ...
        console.log('📥 [sendMessage]:', { roomId, message: message?.substring(0, 50), senderType, senderId });
        ...
        if (!finalSenderId) {
            if (senderType === 'doctor') {
                ...
                finalSenderId = room.patient_id || 'patient-' + Date.now();
            }
        }

        if (!finalSenderId) {
            finalSenderId = senderType === 'doctor' ? 'doctor-' + Date.now() : 'patient-' + Date.now();
        }
        ...
        // 6. استخراج الاسم والرقم من الرسالة (إذا كانت من المريض)
        if (senderType === 'patient') {
            const cleanMessage = message.trim();
            const phoneMatch = cleanMessage.match(/(01[0125]\d{8})/);
            const nameMatch = cleanMessage.match(/(?:اسمي|الاسم|أنا|اسم)\s*[:]?\s*([^\d]+?)(?:\s*رقم|\s*تلفون|\s*موبايل|$)/i);
            ...
            if (patientName || patientPhone) {
                ...
                await supabase
                    .from('chat_rooms')
                    .update({
                        patient_name: patientName || room.patient_name,
                        patient_phone: patientPhone || room.patient_phone
                    })
                    .eq('id', roomId);
            }
        }
